...
paletteHex_com.colorWidget=new Class({Extends:Events,displayRgb:true,displayHsv:true,displayRgbCode:true,updateFormDuringMoveOnPalette:true,posdivElPalette:{},
	...
	initialize:function(propertyArray){
		this.colorHelper=new paletteHex_com.colorUtil();
...
this.parentRef=$(propertyArray.parentRef);
},
setHueSliderPosition:function(hueSliderPosition){
	...
	if(hueSliderPosition=='vertical'){
		...
		var ind=this.objectIndex;setTimeout('paletteHex_com.variableStorage.arrayDSObjects['+ind+'].sliderDiv.style.marginTop=(2 -Math.floor(paletteHex_com.variableStorage.arrayDSObjects['+ind+'].sliderDiv.offsetHeight/2))+"px"',100);}
if(hueSliderPosition=='horizontal'){if(this.sliderDivMain){this.sliderDivHorMain.style.display='block';
this.sliderDivMain.style.display='none';
}}},
setCallbackOnChangeRgb:function(functionName){this.callbackOnChangeRgb=functionName;},setRgbColor:function(rgbColor){var hsv=this.colorHelper.getHsvByRgbCode(rgbColor);this.currentHue=(hsv.hue);this.currentBrightness=(hsv.brightness*100);this.currentSaturation=(hsv.saturation*100);
	this.__changeViewAfterColorChange();},setHue:function(hue){hue=hue+'';
	if(hue.match(/^[0-9]+$/)){while(hue>=360)hue-=360;
this.currentHue=hue;this.__changeViewAfterColorChange();
}},setSaturation:function(saturation){saturation=saturation+'';
if(saturation.match(/^[0-9]+$/)){while(saturation>100)
saturation-=100;this.currentSaturation=saturation;
this.__changeViewAfterColorChange();}},setBrightness:function(brightness){brightness=brightness+'';if(brightness.match(/^[0-9]+$/)){while(brightness>100)
brightness-=100;this.currentBrightness=brightness;
this.__changeViewAfterColorChange();}},getDivElement:function(){return this.divElement;
},
init:function(){this.__createMainDivEl();this.__createdivElPalette();this.__createHueBar();this.__createFormDiv();this.__createHueBarHorizontal();this.__createCustomColorBar();this.__addEvents();this.__setPaletteBgColor();this.__updateHsvInForm();this.__setBgColorPreviewDiv();this.__updateRgbInForm();},



__clearCustomColors:function(){this.customColorIndex=0;this.customColors=[];var els=$$('.ColorBox');for(var i=0;i<els.length;i++){els[i].setStyle('background-color',this.customColorsBgColorOfEmptyBoxes);els[i].removeClass('activeColorBox');}this.__setCustomColorCookie();},__addCustomColor:function(){if(this.customColorIndex==this.maxCustomColors){this.customColorIndex=0;}
var color=this.getColor();this.customColors[this.customColorIndex]=color.rgb;this.__colorizeColorBox(color.rgb);this.customColorIndex++;this.__setCustomColorCookie();},

__colorizeColorBox:function(rgb,colorBox){
	if(!colorBox){var colorBox=$('ColorBox'+this.customColorIndex);}
...
__setCustomColorCookie:function(){
	...
	__getColorBoxesFromCookie:function(){
		...
		if(colorString){
			...
			__ColorBoxClick:function(e){if(e.target.getProperty('rgb')){var rgb=e.target.getProperty('rgb');
			this.fireEvent('selectcustomcolor',e.target.getProperty('rgb'));this.setRgbColor(rgb);}
this.__removeHighlightFromColorBoxes();$(e.target).addClass('activeColorBox');this.customColorIndex=e.target.getProperty('index');},

__createColorBoxes:function(){
	var div=new Element('DIV');
	div.addClass('ColorBoxContainer');
	this.customColorDiv.adopt(div);
	this.customColors=this.__getColorBoxesFromCookie();
	for(var i=0;i<this.maxCustomColors;i++){
		var colorDiv=new Element('DIV');
		colorDiv.id='ColorBox'+i;
		colorDiv.addClass('ColorBox');
		colorDiv.setProperty('index',i);
		colorDiv.addEvent('click',this.__ColorBoxClick.bind(this));
		colorDiv.setStyle('background-color',this.customColorsBgColorOfEmptyBoxes);
		this.customColorDiv.adopt(colorDiv);
		if(this.customColors[i]){this.__colorizeColorBox(this.customColors[i],colorDiv);
			this.customColorIndex++;
		}}
var clearButton=new Element('IMG');
clearButton.addClass('ColorButtonClear');
...
__createCustomColorBar:function(){
	...
	this.customColorDiv.addClass('ColorBoxContainer');
...
__createFormDiv:function(){
	...
	this.divElPreviewDiv.addClass('colorSliderColorPreview');  /*Preview mini side box*/
	...
	table.cellpadding=0;
	table.cellspacing=0;
	table.addClass('colorSliderFormTable');
...
this.divElPaletteCircle.addClass('colorSliderPalette_circle');
...
__setSmallCirclePosition:function(){
	var leftPos=Math.round(this.currentSaturation*(this.paletteSize/100))-this.circleOffsetSize;var topPos=this.paletteSize-Math.round(this.currentBrightness*(this.paletteSize/100))-this.circleOffsetSize;this.divElPaletteCircle.style.left=leftPos+'px';this.divElPaletteCircle.style.top=topPos+'px';this.divElPaletteCircle.className=this.divElPaletteCircle.className.replace(' colorSliderPalette_circleBlack','');
	if(this.currentBrightness>80){this.divElPaletteCircle.className=this.divElPaletteCircle.className+' colorSliderPalette_circleBlack';}},__createHueBar:function(){var ind=this.objectIndex;var mainDiv=new Element('DIV');mainDiv.className='colorSliderHue';this.sliderDivMain=mainDiv;this.divElement.appendChild(mainDiv);this.sliderDiv=new Element('DIV');
this.sliderDiv.addClass('colorSliderHandle');
...
var div=new Element('DIV');
div.addClass('colorSliderHueBarBorder');
mainDiv.appendChild(div);
this.divElHueBar=new Element('DIV');
this.divElHueBar.addClass('colorSliderHueBar');
...
this.sliderDivHor.addClass('colorSliderHandleHorizontal');
this.sliderDivHorMain.appendChild(this.sliderDivHor);this.sliderDivHor.innerHTML='<div><span></span></div>';
setTimeout('paletteHex_com.variableStorage.arrayDSObjects['+ind+'].sliderDiv.style.marginTop=(2 -Math.floor(paletteHex_com.variableStorage.arrayDSObjects['+ind+'].sliderDiv.offsetHeight/2))+"px"',100);
var div=new Element('DIV');div.className='colorSliderHueBarHorizontal_border';
this.sliderDivHorMain.appendChild(div);this.divElHueBarHorizontal=new Element('DIV');
this.divElHueBarHorizontal.addClass('colorSliderHueBarHorizontal');
